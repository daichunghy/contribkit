...
The `cmake-ctest` adapter additionally requires a `CMakeLists.txt` with a
`cmake_minimum_required(VERSION ...)` declaration and `include(CTest)` or
`enable_testing()`; this static check does not execute CMake.
