...
const ALLOWED_TEST_MENTION =
  /\b(npm test|npm run test|pnpm test|yarn test|pytest|python -m pytest|cargo test|go test|dotnet test|ctest|bun test|deno test|swift test|mix test|mvn test)\b/i;
const AI_REQUIRE =
    ...
    const inferred =
      unsafe !== undefined && !unsafeAllowlisted
        ...
        : inferTestCommand({
            ...
            mentionsCtest: /\bctest\b/i.test(contribText),
    ...
    if (!skipRecord && inferred !== undefined) {
      ...
      compiled.push(
        makeRule({
          ...
          check: "command_recorded",
          message:
            "Tests are mentioned but no allowlisted command could be inferred. Record npm test / pytest / cargo test / go test / dotnet test / ctest / bun test / deno test / swift test / mix test / mvn test with exit 0.",
        }),
      );
    }
  }
